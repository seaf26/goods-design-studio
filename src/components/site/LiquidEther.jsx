...
export default function LiquidEther({
  ...
  void main(){ vec3 pos = position; vec2 scale = 1.0 - boundarySpace * 2.0;
  ...
  void main(){ vec3 pos = position; uv = 0.5 + pos.xy * 0.5;
    ...
    void main(){ vec2 pos = position.xy * scale * 2.0 * px + center;
    ...
    const color_frag = `
    precision highp float; uniform sampler2D velocity; uniform sampler2D palette;
    ...
    const poisson_frag = `
    precision highp float; uniform sampler2D pressure; uniform sampler2D divergence;
    uniform vec2 px; varying vec2 uv;
    void main(){
    ...
    const pressure_frag = `
    precision highp float; uniform sampler2D pressure; uniform sampler2D velocity;
    ...
    const viscous_frag = `
    precision highp float; uniform sampler2D velocity; uniform sampler2D velocity_new;
    ...
    class Simulation {
      constructor(options) {
        this.options = {
          ...
          viscous: 30, isBounce: false, dt: 0.014, isViscous: false, BFECC: true,
          ...options
        };
