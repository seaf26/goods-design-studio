...
`;
    ...
    type PassProps = {
      ...
      output?: RenderTarget;
      output0?: RenderTarget;
      output1?: RenderTarget;
    };
    ...
    class ExternalForce extends ShaderPass {
      mouse!: THREE.Mesh;
      constructor(simProps: { cellScale: THREE.Vector2; cursor_size: number; dst: RenderTarget }) {
      ...
      update(
        ...
        let forceX = (Mouse.diff.x / 2) * (props.mouse_force || 0);
        let forceY = (Mouse.diff.y / 2) * (props.mouse_force || 0);
        ...
        let centerSourceX = Mouse.coords.x;
        let centerSourceY = Mouse.coords.y;
        ...
        if (
          ...
          const hoverForce = props.hover_force || 0;
          const hoverOrbit = props.hover_orbit || 0;

          if (hoverForce > 0 || hoverOrbit > 0) {
            const diffLen = Mouse.diff.length();
            const idleBlend = 1 - Math.min(1, diffLen * 78);
            const time = Common.time;
            const angle = time * 2.85;
            const pulse = 0.76 + Math.sin(time * 2.1) * 0.24;
            const strength = hoverForce * pulse * (0.48 + idleBlend * 0.52);
            const orbit = hoverOrbit * (0.72 + idleBlend * 0.28);

            forceX += Math.cos(angle) * strength;
            forceY += Math.sin(angle * 1.14) * strength;
            centerSourceX += Math.cos(angle * 0.7) * orbit;
            centerSourceY += Math.sin(angle * 0.86) * orbit * 0.72;
          }
        }

        const centerX = Math.min(
          Math.max(centerSourceX, -1 + cursorSizeX + cellScale.x * 2),
          1 - cursorSizeX - cellScale.x * 2,
        );
        const centerY = Math.min(
          Math.max(centerSourceY, -1 + cursorSizeY + cellScale.y * 2),
    ...
    class Viscous extends ShaderPass {
      ...
      update(
        ...
        if (!this.uniforms) return;
        let fbo_in: RenderTarget | undefined;
        let fbo_out: RenderTarget | undefined;
    ...
    class Poisson extends ShaderPass {
      ...
      update({ iterations }: { iterations?: number } = {}) {
        let p_in: RenderTarget | undefined;
        let p_out: RenderTarget | undefined;
        const iter = iterations ?? 0;
        for (let i = 0; i < iter; i++) {
          if (i % 2 === 0) {
    ...
    class Pressure extends ShaderPass {
      ...
      update({ vel, pressure }: { vel?: RenderTarget; pressure?: RenderTarget } = {}) {
    ...
    class Simulation {
      ...
      fbo(key: string): RenderTarget {
        const target = this.fbos[key];
        if (!target) throw new Error(`Missing simulation render target: ${key}`);
        return target;
      }
      createShaderPass() {
        ...
        this.viscous = new Viscous({
          ...
          src: this.fbo("vel_1"),
          dst: this.fbo("vel_viscous1"),
          dst_: this.fbo("vel_viscous0"),
          dt: this.options.dt,
        });
        this.divergence = new Divergence({
          cellScale: this.cellScale,
          boundarySpace: this.boundarySpace,
          src: this.fbo("vel_viscous0"),
          dst: this.fbo("div"),
          dt: this.options.dt,
        });
        this.poisson = new Poisson({
          ...
          boundarySpace: this.boundarySpace,
          src: this.fbo("div"),
          dst: this.fbo("pressure_1"),
          dst_: this.fbo("pressure_0"),
        });
        this.pressure = new Pressure({
          cellScale: this.cellScale,
          boundarySpace: this.boundarySpace,
          src_p: this.fbo("pressure_0"),
          src_v: this.fbo("vel_viscous0"),
          dst: this.fbo("vel_0"),
          dt: this.options.dt,
        });
      }
      ...
      update() {
        ...
        this.externalForce.update({
          ...
          hover_force: this.options.hover_force,
          hover_orbit: this.options.hover_orbit,
          cellScale: this.cellScale,
        });
        let vel: RenderTarget = this.fbo("vel_1");
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      loop(now = performance.now()) {
        ...
        const activeFps =
          now - this.lastUserInteraction < this.interactionBoostMs
      ...
      setFrameBudget(nextMaxFps: number, nextInteractiveMaxFps?: number) {
        ...
        this.interactiveMaxFps =
          Number.isFinite(interactiveFps) && interactiveFps > 0
